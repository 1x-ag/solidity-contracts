...
import "./FulcrumUtils.sol";


contract ProtocolFulcrum is FulcrumUtils, OracleChainLink {
    using SafeMath for uint256;
    using UniversalERC20 for IERC20;

    function collateralAmount(IERC20 token) public returns(uint256) {
        return _getIToken(token).assetBalanceOf(address(this));
    }

    function borrowAmount(IERC20 token) public returns(uint256) {
        return _getIToken(token).borrowBalanceCurrent(address(this));
    }
